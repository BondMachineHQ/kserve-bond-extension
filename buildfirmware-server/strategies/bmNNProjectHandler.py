...
class BMNeuralNetworkProjectHandler:
    ...
    def build(self):
        ...
        cmdToExec = f'cd {self.request.uuid} && bmhelper create --project_name {self.projectName} --board {self.targetBoard} --project_type {self.projectType} --n_inputs {self.nInputs} --n_outputs {self.nOutputs} --source_neuralbond {self.neuralNetwork}'
        try:
    ...
    def exec(self):
        ...
        cmdToExec = f'cd {self.request.uuid}/{self.projectName} && make hdl'
        try:
